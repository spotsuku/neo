...
- `POST /api/admin/bulk-registration` - CSV一括登録（事務局限定）
...
5. **CSV一括登録**: 「CSV一括登録」ボタンでファイルアップロード機能テスト
6. **統計表示**: 登録統計・地域別データ・補完率確認
7. **データエクスポート**: 「データエクスポート」で統計データCSV出力

## 最終更新
- **バージョン**: v2.2.0（ハイブリッド登録システム追加）
- **更新日**: 2024-08-24
- **新機能**: 事務局事前登録→本人プロフィール補完→承認フロー完全実装
